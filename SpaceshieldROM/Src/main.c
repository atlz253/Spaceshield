...
int main(void)
{
    ...
    RGB_POWER(5, 0, 0);
    ssd1306_Init();           // процедура настройки дисплея
    ssd1306_Fill(Black) ;               // очистка буфера дисплея
    ssd1306_SetCursor(1,0);
    ssd1306_WriteString("Welcome back!", Font_7x10, White);
		ssd1306_SetCursor(1,16);
		ssd1306_WriteString("Mod by Alekseev F.", Font_7x10, White);
  ...
  /* Infinite loop */
    ...
    while (1)
    {
			HAL_Delay(15); // обновление экрана раз в 15 миллисекунд (~60 кадров)
