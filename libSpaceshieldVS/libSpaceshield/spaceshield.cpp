...
void clear_display(display_color color)
{
	display_point point;
	...
	for (size_t j = 0; j < 64; j++)
	{
		...
		{
			surface[i][j] = (color == DISPLAY_COLOR_WHITE) ? DISPLAY_COLOR_BLACK : DISPLAY_COLOR_WHITE; // ставим единицу, чтобы гарантированно пиксель потух
...
void display_fill(display_color color)
{
	...
	if (hid_send_feature_report(handle, buf, 2) == -1) {
		printf("DISPLAY_FILL_FUNCTION_CODE not found, trying pixel-by-pixel cleaning.\n");
		clear_display(color);
	}
}
